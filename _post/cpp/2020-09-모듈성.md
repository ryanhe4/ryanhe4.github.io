---
excerpt:Modularity
header:
    overlay_image: /assets/images/header_image_1.jpg
title: "[c++] Modularity"
categories: c++
date: '2020-09-02 20:30:00 +0900'
toc: true  
toc_sticky: true
---
C++의 모듈성에 대해 학습한다.

C++ 프로그램은 함수, 사용자 정의타입, 클래스 계층, 템플릿 등많은 분리된 개발 부분으로 구성되어있다.
가장 중요한 단계는 인터페이스 부분과 그것의 구현을 구별하는 것이다.  
언어 단계에서 C++는 인터페이스를 선언으로 나타낸다. 선언은 사용하는데 필요한 모든  함수나 타입을 지정한다.
예:
```c++
double sqrt(double); // the square root function takes a double and returns a double

class Vector {
public:
    Vector(int s);
    double& operator[](int i);
    int size();
private:
    double* elem; // elem points to an array of sz doubles
    int sz;
};
```
여기서 중요한점은 함수 몸체(함수 정의)는 다른곳에 있다.

# 분할 컴파일
C++는 분할 컴파일 개념을 지원한다.
타입과 함수들의 정의는 소스파일을 분리하고 각각 컴파일 된다.
이러한 분리는 컴파일 시간을 최소화하고 프로그램의 완전히 논리적 구별 부분을 강요한다

인터페이스는 **.h** 파일에 저장한다.
구현은 **.cpp** 파일에 저장한다. (#include .h => 인터페이스 제공)

# 모듈(C++20)
**#include**의 사용은 굉장히 오래되고, 에러가 자주발생하고, 프로그램 구성하는데 비싸다.

# 네임스페이스
# 에러 처리
# 함수 인자와 반환 값