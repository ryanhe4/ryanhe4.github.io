...
Wake-on: g
```
Wake-on 부분이 d나 다른 설정으로 돼 있을 경우 다음 명령어로 설정해 줍시다.
```console
...
```
{:.note title="/etc/modprobe.d/r8168.conf"}
...
WakeOnLan=magic
```
{:.note title="/etc/sytstemd/network/50-wired.link"}
...
```
{:.note title="/etc/udev/rules.d/81-wol.rules"}

## TLP
자신의 시스템에서 TLP가 작동되고 있으면 TLP의 기본설정 때문에 WOL 기능이 종료될 수 있습니다. TLP설정 또한 해줍시다.
* 제가 설치한 Manjaro KDE는 기본적으로 tlp가 설치돼 있었습니다.

```coffeescript
WOL_DISABLE=N
```
{:.note title="/etc/tlp.conf"}
