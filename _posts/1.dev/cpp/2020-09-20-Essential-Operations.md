...
할당은 복사 또는 이동 할당 연산자를 사용합니다. 원칙적으로, 할당이 아닌 다른 경우 복사나 이동 생성자를 사용합니다. 
...
default를 통해 기본 복사 생성자, 또는 이동 생성자를 만들 수 있습니다.
```cpp
class Y {
public:
    Y(Sometype);
...
클래스가 포인터 멤버를 가지고있으면 복사 또는 이동 연산에 대해서 명시적으로 하는것은 좋은 생각 입니다. 그 이유는 포인터는 클래스가 삭제해야 할것을 
가르킬 수 있기 때문 입니다. 이 경우 기본 멤버 별 복사가 잘못됩니다.<br>
좋은 규칙은 필수연산자 모두를 정의하거나 또는 아무것도 하지않는것(기본값 사용) 입니다.
```cpp
struct Z {
    Vector v;
    string s;
...
`=default`를 보완하기 위해 작업이 생성되지 않음을 나타내는 `=delete`가 있습니다. 클래스 계층에서 기본 클래스는 멤버별 복사 연산을
 허락하지않는 고전적인 예 입니다. 
 ```cpp
class Shape {
public:
...
complex z2 = z1*2;
``` 
이러한 암시적 변환은 떄떄로 이상적이지만, 항상 그런것은 아닙니다. 
```cpp
Vector v1 = 7;  // v1은 7개의 인자를 갖습니다.
```
이것은 나쁘게 생각되고 표준라이브러리 `vecotr`는 이러한 `int` - `vector` 변환을 허락하지 않습니다.<br>
이러한 문제를 피하기위한 방법은 명시적인 변환만을 허락하는것 입니다. 생성자를 다음과 같이 정의할수 있습니다.
```cpp
class Vector{
public:
    explicit Vector(int s); // no implicit conversion fron int to Vector
    //...
}
```

## 멤버 초기화
클래스의 데이터 멤버가 정의될 때,기본 멤버 초기화라고 불리는 기본적인 초기화 제공할 수 있습니다. 
```cpp
class complex {
    double re = 0;
...
기본적인 복사의 의미는 멤버별 복사입니다: 모든 멤버의 복사. 예를 들어 `complex`를 사용하면
```cpp
...
``` 
클래스를 디자인 할 때 항상 객체가 복사 될 수 있는지와 어떻게 복사되는지 고려해야합니다.
### 컨테이너 복사
클래스가 리소스 핸들(resource handle)일때, 포인터를 사용할때 기본적인 멤버별 복사는 재앙 입니다. 예를들어 기본 복사를 하면
`Vector`의 복사본은 원본과 같은 위치를 가리킬 것 입니다. <br>
`Vector`가 소멸자를 가지고 있다는 사실은 기본 복사 의미가 잘못되었고 컴파일러는 이 예제에 최소한의 경고를 할것이라는 강력한 힌트입니다.
우리는 더 나은 복사 의미를 정의 해야 합니다.<br>

클래스의 객체의 복사는 두 멤버에의해 정의됩니다: 복사 생성자와 복사 대입 연산자(copy assignment)
```cpp
class Vector {
private:
...
### 컨테이너 이동 
복사생성자와 복사 대입연산자를 정의함으로써 복사를 다룰수 있습니다. 하지만 복사는 큰 컨테이너에서는 비용이 비쌀 수 있습니다.
우리는 객체를 함수에 넘길때 참조를 사용하여 비싼 복사를 회피하지만, 결과로써 지역 객체로 참조자를 반환받지 못합니다.
...
`&&`는 'rvalue 참조'를 의미하며, rvalue를 바인딩 할 수 있는 참조입니다. "rvalue"라는 단어는 "lvalue"를 보완하기위한 것으로 대략
 "할당의 왼쪽에 나타날 수있는 것"을 의미합니다. 따라서 rvalue는 – 첫 번째 근사치 – 함수 호출에 의해 반환 된 정수와 같이 할당 할 수없는 값입니다.
