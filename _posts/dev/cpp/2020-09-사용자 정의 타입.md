...
high-level의 편리한 기능들을 제공하지 않는다. 대신 C++은 정교한 추상 메커니즘을 제공한다.
...
```
구조체 Vector는 int 와 double*으로 구성되어있다.
...
# 클래스
클래스는 사용자 정의 타입에서 데이터와 연산자, 기능들을 연결이 필요할 때 사용한다.
...
```
생성자(constructor) : 클래스 이름과 같은 멤버 함수, 클래스 객체의 초기화를 보장
...
```
위의 예제에서 p와 i는 동시에 사용되지 않는다 이런경우 p와 i를 union으로 정의해서 사용한다
```c++
union Value {
    Node* p;
    int i;
...
```
많은 경우 variant의 사용이 union보다 좋은 경우가 많다.

# Enumerations
클래스에서는 C++는 간단한 사용자 정의 타입을 위한 열거형을 제공한다
```c++
enum class Color { red, blue, green};
enum class Traffic_lighht { green, yellow ,red};
Color col = Color::red;
Traffic_light light = Traffic_light::red;
```
여기서 두가지 열거클래스의 red는 다르다 (Color::red != Traffic_light::red)
...
Color y {6};
```
기본적으로 enum class 는 할당, 초기화, 비교 연산자가 정의된다. 
하지만 열거형은 사용자 정의 타입이라 연산자를 정의 할수 있다.
